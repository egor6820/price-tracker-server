...
# ---- Config: which domains need JS rendering (playwright) by default ----
...
# ---- Playwright: start once on app startup, reuse browser, semaphore to limit concurrency ----
PLAYWRIGHT_ENABLED = os.getenv("ENABLE_PLAYWRIGHT", "true").lower() != "false"
PLAYWRIGHT_MAX_PARALLEL = int(os.getenv("PLAYWRIGHT_MAX_PARALLEL", "1"))  # default 1 to save memory

@app.on_event("startup")
def startup_playwright():
    if not PLAYWRIGHT_ENABLED:
        print("Playwright disabled by env var.")
        app.state.play = None
    ...
    try:
        ...
        # lighter args to reduce memory usage
        ...
        print("Playwright started and browser created. Max parallel:", PLAYWRIGHT_MAX_PARALLEL)
...
def shutdown_playwright():
    try:
        ...
        if getattr(app.state, "play", None):
            app.state.play.stop()
        print("Playwright shutdown complete")
    except Exception as e:
        print("Playwright shutdown error:", e)
        traceback.print_exc()

# ---- Playwright extraction (uses shared browser + semaphore) ----
def extract_with_playwright_direct(url: str, domain_cfg: dict | None = None, wait_for_price_sec: int = 20):
    ...
    if sem:
        ...
        if not acquired:
            raise RuntimeError("Timeout while waiting for Playwright semaphore")
    else:
        acquired = False
...
# ---- Robust fetch: first try requests for non-dynamic domains, else playwright attempts then requests fallback ----
def robust_fetch_html(url: str, domain_cfg: dict | None = None, playwright_attempts: int = 2, requests_attempts: int = 3):
    domain_is_dynamic = False
    for d in DYNAMIC_DOMAINS:
        if d in url:
            domain_is_dynamic = True
            break

    # If domain is not dynamic -> try requests first (cheaper)
    if not domain_is_dynamic:
        for i in range(requests_attempts):
            ...
            time.sleep(0.5 * (i+1))
        # if requests didn't work, attempt playwright below (if available)

    # 1) Playwright attempts (only if available)
    if getattr(app.state, "browser", None):
...
def parse_product(req: ParseRequest):
    ...
    try:
        ...
        # --- robust fetch: playwright or requests depending on domain and availability ---
        try:
                ...
                for sel in domain_cfg.get("price", []):
                    ...
                    if tag:
                        ...
                        if cp:
                            txt = tag_text_or_attr(tag)
                            try:
                                cond = contains_currency(txt) or float(cp) >= 20 or re.search(r"(price|product-price|грн|uah)", " ".join(filter(None, [tag.get("class") and " ".join(tag.get("class")), tag.get("id") or ""])), flags=re.I)
                            except Exception:
                                cond = contains_currency(txt) or float(cp) >= 20
                            if cond:
                                currentPrice = cp
                                break
            if not name:
